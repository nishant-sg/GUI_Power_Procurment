*POWER PROCUREMENT FOR THE STATE OF RAJASTHAN YEAR 2021
*THERMAL
*HYDRO
*SOLAR
*WIND
...
$CALL GDXXRW Rajj_prelim_data.xlsx Par=Gen_Data rng=Gdetails1!A1:H70 Rdim=1 Cdim=1
$GDXIN Rajj_prelim_data.gdx
$LOAD Gen_Data
display Gen_Data ;
Parameter demand1(t,*);
$CALL GDXXRW Rajj_prelim_data.xlsx  Par=demand1 rng=Demand_Data!A1:B97 Rdim=1 Cdim=1
$GDXIN Rajj_prelim_data.gdx
$LOAD demand1
display demand1;
Parameter PV_Act(t,*);
$CALL GDXXRW Rajj_prelim_data.xlsx  Par=PV_Act rng=PV_Data!A1:B97 Rdim=1 Cdim=1
$GDXIN Rajj_prelim_data.gdx
$LOAD PV_Act
display PV_Act;
Parameter Wind_Act(t,*);
$CALL GDXXRW Rajj_prelim_data.xlsx  Par=Wind_Act rng=Wind_Data!A1:B97 Rdim=1 Cdim=1
$GDXIN Rajj_prelim_data.gdx
$LOAD Wind_Act
display Wind_Act;
...
$CALL GDXXRW Rajj_prelim_data.xlsx  Par=PX_Data rng=PX_Data!A1:E16 Rdim=1 Cdim=1
$GDXIN Rajj_prelim_data.gdx
$LOAD PX_Data
display PX_Data ;
Parameter PX_Cost(PX_no,*);
$CALL GDXXRW Rajj_prelim_data.xlsx  Par=PX_Cost rng=PX_Cost!A1:B16  Rdim=1 Cdim=1
$GDXIN Rajj_prelim_data.gdx
$LOAD PX_Cost
display PX_Cost;
parameter PX_Avail(PX_no,*);
$CALL GDXXRW Rajj_prelim_data.xlsx  Par=PX_Avail rng=PX_CA!A1:CS16 Rdim=1 Cdim=1
$GDXIN Rajj_prelim_data.gdx
$LOAD PX_Avail
display PX_Avail;
parameter DSM_cost(t,*);
$CALL GDXXRW Rajj_prelim_data.xlsx  Par=DSM_cost rng=DSM_Data!C1:D97 Rdim=1 Cdim=1
$GDXIN Rajj_prelim_data.gdx
$LOAD DSM_cost
display DSM_cost;
parameter REC_Data(t,*);
$CALL GDXXRW Rajj_prelim_data.xlsx  Par=REC_Data rng=REC_Data!A1:C97 Rdim=1 Cdim=1
$GDXIN Rajj_prelim_data.gdx
$LOAD REC_Data
display REC_Data;
parameter DAM_Data(t,*);
$CALL GDXXRW Rajj_prelim_data.xlsx  Par=DAM_Data rng=DAM_Data!A1:B97 Rdim=1 Cdim=1
$GDXIN Rajj_prelim_data.gdx
$LOAD DAM_Data
display DAM_Data;
parameter RTM_Data(t,*);
$CALL GDXXRW Rajj_prelim_data.xlsx  Par=RTM_Data rng=RTM_Data!A1:B97 Rdim=1 Cdim=1
$GDXIN Rajj_prelim_data.gdx
$LOAD RTM_Data
display RTM_Data;
parameter Risk_Data(t,*);
$CALL GDXXRW Rajj_prelim_data.xlsx  Par=Risk_Data rng=Risk_Data!A2:P97 Rdim=1 Cdim=1
$GDXIN Rajj_prelim_data.gdx
$LOAD Risk_Data
display Risk_Data;
parameter alpha;
...
OF.. Total_Cost =e=  Gen_cost + LS_cost + DSM_costt + PXX_cost + S_REC_cost + NS_REC_cost + PXXDAM_cost + PXXRTM_cost;
...
powerbal1(t).. sum(Gen,G(Gen,t))+ E(t) + DSM(t) + DAM(t) + RTM(t) + PV_sch(t) + Wind_sch(t) =g= demand1(t,'1');
...
TPPC1.. Total_PPC1 =e= [365*Total_cost] + Conv_FC ;
TPPC2.. Total_PPC2 =e= [365*Final_cost] + Conv_FC ;
...
alpha$(ord(counter)=1)=0.00000001;
alpha$(ord(counter)=2)=0.00000005;
alpha$(ord(counter)=3)=0.00000010;
alpha$(ord(counter)=4)=0.00000050;
alpha$(ord(counter)=5)=0.0000040;
alpha$(ord(counter)=6)=0.0000080;
...
solve Raj_02 using MINLP minimizing Final_cost;
