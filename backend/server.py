...
@app.route('/genOutput', methods=['POST'])
def updateIEXData():
    print(request.get_json(),int(time.time()))
    ...
    shutil.copyfile(f'StateData/{state}.gms',f'{sessionId}/{state}.gms')
    ...
    shutil.copyfile(f'StateData/{state}.lst',f'{sessionId}/{state}.lst')


    inputFile = f'{sessionId}/{state}.lst'
