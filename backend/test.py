...
thermal =True
hydro = False
os.makedirs(f'backend/{sessionId}', exist_ok=True)
path1 = 'backend/StateData/CommonData.xlsx'
path2 = f'backend/{sessionId}/input.xlsx'
...
path1 = f'backend/StateData/{state}.xlsx'
wb1 = xl.load_workbook(filename=path1)
...
wb2.create_sheet(index=0,title = "Demand_Data") 
sheet = wb2.active 
...
if not solar:
    ...
    for i in range(2,98):
        sheet["B"+str(i)] = 0
if not hydro:
    sheet = wb2['Hydro_Data']
    for i in range(2,12):
        for j in range(66,86):
            sheet[chr(j)+str(i)] = 0
if not thermal:
    sheet = wb2['Gdetails']
    for i in range(2,50):
...
shutil.copyfile(f'backend/StateData/{state}.gms',f'backend/{sessionId}/{state}.gms')

command = f'"{os.path.dirname(os.path.abspath(__file__))}\{sessionId}\{state}.gms" -o "{os.path.dirname(os.path.abspath(__file__))}\{sessionId}\{state}.lst"'
command = re.sub("\\\\","/", command)
# subprocess.run(f"C:/GAMS/45/gams {command}")
shutil.copyfile(f'backend/StateData/{state}.lst',f'backend/{sessionId}/{state}.lst')


inputFile = f'backend/{sessionId}/{state}.lst'
...
for i in range(1,iteration):  
    ...
    for _ in range(1,97):
        ...
        print(data[0],data[2])
        if i ==1:
            r1.append(int(data[2]))
        if i ==2:
            r2.append(int(data[2]))
    line = lines.pop(0)
    print("here -> ",line)
...
print(f"C:/GAMS/45/gams {command}")
